...
// withFilePreviewURL 将库里存的相对路径或 MinIO 直链规范为客户端可直接 GET 的绝对 URL（与充值渠道 qr_image 逻辑一致）。
func (w *Workplace) withFilePreviewURL(c *wkhttp.Context, raw string) string {
	raw = strings.TrimSpace(raw)
	if raw == "" {
		return ""
	}
	cfg := w.ctx.GetConfig()
	ext := strings.TrimSpace(cfg.External.APIBaseURL)
	apiBase := wkutil.FilePreviewAPIBaseURL(c.Request, ext)
	if apiBase == "" {
		apiBase = ext
	}
	// 与钱包充值渠道一致：返回完整绝对 URL。勿再转为根相对路径；原生 App 的 Image 无法解析以 / 开头的相对地址。
	return wkutil.FullAPIURLForFilePreview(apiBase, raw, cfg.Minio.UploadURL, cfg.Minio.DownloadURL)
}
	...
	if len(records) > 0 {
		...
		if len(apps) > 0 {
			...
			for _, app := range apps {
				...
				appResp.Icon = w.withFilePreviewURL(c, app.Icon)
		...
		for _, m := range models {
			...
			if app != nil {
				list = append(list, &appDetailResp{
					AppID:       m.AppID,
					SortNum:     m.SortNum,
					Icon:        w.withFilePreviewURL(c, app.Icon),
					Name:        app.Name,
	...
	if len(models) > 0 {
		for _, m := range models {
			list = append(list, &bannerResp{
				BannerNo:    m.BannerNo,
				Cover:       w.withFilePreviewURL(c, m.Cover),
				Title:       m.Title,
