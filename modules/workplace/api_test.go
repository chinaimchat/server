...
func TestUserGetApp(t *testing.T) {
	...
	req.Header.Set("token", token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"name":"龙虾IM"`))
}

func TestUserGetAppIconUsesFilePreviewAndEscapedURL(t *testing.T) {
	s, ctx := testutil.NewTestServer()
	wp := New(ctx)
	wm := NewManager(ctx)
	err := testutil.CleanAllTables(ctx)
	...
	err = wm.db.insertAPP(&appModel{
		AppID:       appID,
		Icon:        "file/preview/report/1776142170892/2026-04-14 11.54.18.jpg",
		Name:        "龙虾IM",
		Description: "龙虾IM让信息传递更简单",
		JumpType:    0,
		AppRoute:    "http://www.githubim.com",
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("GET", "/v1/workplace/app", nil)
	req.Host = "example.com:8090"
	req.Header.Set("token", token)
	s.GetRoute().ServeHTTP(w, req)

	body := w.Body.String()
	assert.Equal(t, true, strings.Contains(body, `"icon":"http://example.com:8090/v1/file/preview/report/1776142170892/2026-04-14%2011.54.18.jpg"`))
	assert.Equal(t, false, strings.Contains(body, "2026-04-14 11.54.18.jpg"))
}
...
func TestGetRecord(t *testing.T) {
	...
	req.Header.Set("token", token)
	s.GetRoute().ServeHTTP(w, req)
	assert.Equal(t, true, strings.Contains(w.Body.String(), `"name":"小龙虾"`))
}
