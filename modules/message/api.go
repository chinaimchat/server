...
func (m *Message) sendMessage(channelID string, channelType uint8, fromUID string, payload map[string]interface{}) error {
	if err := m.validateWalletMessagePayload(fromUID, channelID, channelType, payload); err != nil {
		m.Warn("blocked forged wallet payload on public send endpoint",
			zap.Error(err),
			zap.Int("content_type", m.contentType(payload)),
			zap.String("from_uid", fromUID),
			zap.String("channel_id", channelID),
			zap.Uint8("channel_type", channelType),
		)
		return errors.New("非法钱包消息")
	}
...
func (m *Message) isPrivilegeMutualDeleteEnabled(uid string, channelType uint8) (bool, error) {
	type row struct {
		UID                  string `db:"uid"`
		MutualDeletePersonOn int    `db:"mutual_delete_person_on"`
		MutualDeleteGroupOn  int    `db:"mutual_delete_group_on"`
	}
	var rows []*row
	_, err := m.ctx.DB().Select("uid,mutual_delete_person_on,mutual_delete_group_on").
		From("user_privilege").
		Where("uid=?", uid).
		Limit(1).
		Load(&rows)
	...
	if len(rows) == 0 || rows[0] == nil || strings.TrimSpace(rows[0].UID) == "" {
		return false, nil
	}
	if channelType == common.ChannelTypePerson.Uint8() {
		return rows[0].MutualDeletePersonOn == 1, nil
	}
	if channelType == common.ChannelTypeGroup.Uint8() {
		return rows[0].MutualDeleteGroupOn == 1, nil
	}
	return false, nil
}
...
func (m *Message) delete(c *wkhttp.Context) {
	...
	localDeleteReqs := make([]*deleteReq, 0, len(reqs))
	mutualDeleteReqs := make([]*deleteReq, 0, len(reqs))
	for _, req := range reqs {
		enabled, err := m.isPrivilegeMutualDeleteEnabled(loginUID, req.ChannelType)
		if err != nil {
			m.Error("查询特权双删开关失败", zap.Error(err), zap.String("uid", loginUID))
	...
	for _, req := range mutualDeleteReqs {
		err := m.mutualDeleteMessageByReq(req, loginUID, true)
		if err != nil {
			m.Error("特权双删失败", zap.Error(err), zap.String("uid", loginUID), zap.String("channelID", req.ChannelID), zap.Uint8("channelType", req.ChannelType), zap.Uint32("messageSeq", req.MessageSeq))
			c.ResponseError(err)
			return
		}
	}

	if len(localDeleteReqs) == 0 {
		c.ResponseOK()
		return
	}
	...
	for _, req := range localDeleteReqs {
...
func (m *Message) genMessageExtraSeq(_ string) int64 {
