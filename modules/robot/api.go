...
func (rb *Robot) Route(r *wkhttp.WKHttp) {
	...
	rb.syncSystemRobotMenuRemarksFromDefaults()
}

// syncSystemRobotMenuRemarksFromDefaults 将内置系统机器人菜单的 remark 写回数据库，与代码中 systemRobotMap 一致；有变更时提升 version 以便客户端同步。
func (rb *Robot) syncSystemRobotMenuRemarksFromDefaults() {
	robotID := rb.ctx.GetConfig().Account.SystemUID
	m, err := rb.db.queryRobotWithRobtID(robotID)
	if err != nil {
		rb.Error("同步系统机器人菜单：查询机器人失败", zap.Error(err))
		return
	}
	if m == nil {
		return
	...
	var changed bool
	for _, item := range systemRobotMap {
		n, err := rb.db.updateMenuRemark(robotID, item.CMD, item.Remark)
		if err != nil {
			rb.Error("同步系统机器人菜单说明失败", zap.String("cmd", item.CMD), zap.Error(err))
			continue
		}
		if n > 0 {
			changed = true
	...
	m.Version = rb.ctx.GenSeq(common.RobotSeqKey)
	if err := rb.db.updateRobot(m); err != nil {
		rb.Error("同步系统机器人菜单后更新版本失败", zap.Error(err))
	}
}
...
// 是否允许发送消息到频道
func (rb *Robot) allowSendToChannel(_ string, _ uint8) bool {
	// TODO：待完善
	return true
}
