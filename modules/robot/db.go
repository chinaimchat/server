...
// updateMenuRemark 按机器人与命令更新菜单说明；若内容与库中一致，MySQL 通常返回 RowsAffected=0。
func (d *robotDB) updateMenuRemark(robotID, cmd, remark string) (int64, error) {
	r, err := d.session.Update("robot_menu").Set("remark", remark).Set("updated_at", dbr.Now).Where("robot_id=? and cmd=?", robotID, cmd).Exec()
	if err != nil {
		return 0, err
	}
