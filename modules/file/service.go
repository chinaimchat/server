...
type IService interface {
	...
	// OpenObjectDirect 从存储直连读对象（如 MinIO），供 /file/preview 代理；不支持时返回 err
	// 返回 (reader, contentType, sizeBytes, error)
	OpenObjectDirect(ph string) (io.ReadCloser, string, int64, error)
	// StatObjectDirect 仅查询对象元数据（用于 HEAD 预览，避免整包下载）
	StatObjectDirect(ph string) (size int64, contentType string, err error)
	// DownloadURLForServerFetch 供本进程内向对象存储发起 HTTP GET（如 OpenObject 失败后的回退）。
	// MinIO 下强制使用 UploadURL（容器内可达），避免 DownloadURL 配成公网 IP 时服务端 dial 失败却返回 JSON，客户端 Glide 误当图片解码。
	DownloadURLForServerFetch(path string, filename string) (string, error)
}
...
func (s *Service) DownloadURLForServerFetch(path string, filename string) (string, error) {
	if sm, ok := s.uploadService.(*ServiceMinio); ok {
		return sm.DownloadURLForServerFetch(path, filename)
	}
	return s.uploadService.DownloadURL(path, filename)
}
...
func (s *Service) OpenObjectDirect(ph string) (io.ReadCloser, string, int64, error) {
	if sm, ok := s.uploadService.(*ServiceMinio); ok {
		return sm.openObject(ph)
	}
	return nil, "", 0, errors.New("direct open not supported")
}

func (s *Service) StatObjectDirect(ph string) (int64, string, error) {
	if sm, ok := s.uploadService.(*ServiceMinio); ok {
		return sm.statObject(ph)
	}
	return 0, "", errors.New("direct stat not supported")
}
...
func uploadFile(uploadURL, fileName string, copyFileWriter func(io.Writer) error) (map[string]interface{}, error) {
	...
	if err != nil {
		log.Error("构建formFile失败！", zap.Error(err))
		return nil, err
	}
	err = copyFileWriter(fileWriter)
	if err != nil {
		log.Error("复制文件内容失败！", zap.String("uploadURL", uploadURL), zap.Error(err))
		return nil, err
	}
	bodyWriter.Close()
	fRequest, err := http.NewRequest("POST", uploadURL, body)
	if err != nil {
		log.Error("创建上传请求失败！", zap.String("uploadURL", uploadURL), zap.Error(err))
		return nil, err
	}
	...
	if err != nil {
		log.Error("上传文件失败！", zap.String("uploadURL", uploadURL), zap.Error(err))
		return nil, err
	}
	if resp == nil {
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		log.Error("文件上传返回状态有误！", zap.Int("status", resp.StatusCode), zap.String("uploadURL", uploadURL), zap.String("fileName", fileName))
		return nil, errors.New("文件上传返回状态有误！")
	}
	defer resp.Body.Close()
	respData, err := io.ReadAll(resp.Body)
	if err != nil {
		log.Error("读取上传返回的数据失败！", zap.Error(err))
		return nil, err
	}
	var resultMap map[string]interface{}
	err = util.ReadJsonByByte(respData, &resultMap)
	if err != nil {
		log.Error("上传返回的json格式有误！", zap.String("uploadURL", uploadURL), zap.String("fileName", fileName), zap.String("resp", string(respData)), zap.Error(err))
		return nil, err
	}
