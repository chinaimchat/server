...
func (s *Service) showLastOfflineOn() bool {
	appconfig, err := s.commonService.GetAppConfig()
	if err != nil {
		s.Warn("查询应用配置失败，默认隐藏上次在线时间", zap.Error(err))
		return false
	}
	if appconfig == nil {
		return true
...
func (s *Service) GetUserDetails(uids []string, loginUID string) ([]*UserDetailResp, error) {
	...
	for _, userDetail := range userDetails {
		...
		userDetailResps = append(userDetailResps, NewUserDetailResp(userDetail, nameRemark, loginUID, sourceFrom, online, s.sanitizeLastOffline(lastOffline, showLastOfflineOn), deviceFlag, follow, status, beDeleted, beBlacklist, setting, vercode))
	}
...
func (s *Service) AddUser(user *AddUserReq) error {
	...
	if uid == "" {
		uid, err = s.db.AllocateStagedNumericUID(tx)
		if err != nil {
			tx.Rollback()
			s.Error("分配UID失败", zap.Error(err))
			return err
		}
	}
	...
	if err = tx.Commit(); err != nil {
		tx.Rollback()
		s.Error("添加用户提交失败", zap.Error(err))
		return err
	}
	return nil
}
