...
func (f *Friend) friendApply(c *wkhttp.Context) {
	...
	relationLoginUser, err := f.db.queryWithUID(fromUID, req.ToUID)
	if err != nil {
		f.Error("查询好友关系失败！", zap.Error(err), zap.String("uid", fromUID), zap.String("toUid", req.ToUID))
		c.ResponseError(errors.New("查询好友关系失败！"))
		return
	}
	relationToUser, err := f.db.queryWithUID(req.ToUID, fromUID)
	if err != nil {
		f.Error("查询好友关系失败！", zap.Error(err), zap.String("uid", req.ToUID), zap.String("toUid", fromUID))
		c.ResponseError(errors.New("查询好友关系失败！"))
		return
	}
	relationExistsLoginUser := relationLoginUser != nil
	relationExistsToUser := relationToUser != nil
	...
	appconfig, cfgErr := f.commonService.GetAppConfig()
	if cfgErr != nil {
		f.Error("查询全局配置失败", zap.Error(cfgErr))
		c.ResponseError(errors.New("查询全局配置失败"))
		return
	}
	if appconfig != nil && appconfig.PrivilegeOnlyAddFriendOn == 1 {
		privilegeOnlyM, pErr := f.privilegeDB.queryByUID(fromUID)
		if pErr != nil {
			f.Error("查询特权用户失败", zap.Error(pErr), zap.String("uid", fromUID))
			c.ResponseError(errors.New("查询特权用户失败"))
			return
		}
		if privilegeOnlyM == nil || privilegeOnlyM.UID == "" {
			c.ResponseError(errors.New("仅特权账号可以添加好友"))
			return
		}
	}
	if appconfig != nil && appconfig.FriendApplyAutoAcceptOn == 1 {
		privilegeM, pErr := f.privilegeDB.queryByUID(fromUID)
		if pErr != nil {
			f.Error("查询特权用户失败", zap.Error(pErr), zap.String("uid", fromUID))
			c.ResponseError(errors.New("查询特权用户失败"))
			return
		...
		if privilegeM != nil && privilegeM.UID != "" {
			tx, txErr := f.ctx.DB().Begin()
			if txErr != nil {
				f.Error("开启事务失败", zap.Error(txErr))
				c.ResponseError(errors.New("开启事务失败"))
				return
			}
			defer func() {
			...
			if !relationExistsToUser {
				err = f.db.InsertTx(&FriendModel{
					...
					SourceVercode: req.Vercode,
				}, tx)
				if err != nil {
					tx.RollbackUnlessCommitted()
					c.ResponseError(errors.New("自动通过添加好友失败"))
					return
				}
			} else {
				err = f.db.updateRelationshipTx(req.ToUID, fromUID, 0, 0, "", version, tx)
				if err != nil {
					tx.RollbackUnlessCommitted()
					c.ResponseError(errors.New("自动通过添加好友失败"))
					return
			...
			if !relationExistsLoginUser {
				err = f.db.InsertTx(&FriendModel{
					...
					IsDeleted:     0,
					Vercode:       fmt.Sprintf("%s@%d", util.GenerUUID(), common.Friend),
					SourceVercode: fmt.Sprintf("%s@%d", util.GenerUUID(), common.Friend),
				}, tx)
				if err != nil {
					tx.RollbackUnlessCommitted()
					c.ResponseError(errors.New("自动通过添加好友失败"))
				...
				err = f.db.updateRelationshipTx(fromUID, req.ToUID, 0, 0, "", version, tx)
				if err != nil {
					tx.RollbackUnlessCommitted()
					c.ResponseError(errors.New("自动通过添加好友失败"))
					return
			...
			// 自动通过也要走 FriendSure 事件：用于互加 IM 白名单，否则会出现“已是好友但仍提示开启好友验证”
			eventID, eErr := f.ctx.EventBegin(&wkevent.Data{
			...
			if eErr != nil {
				tx.RollbackUnlessCommitted()
				f.Error("发送好友确认事件失败", zap.Error(eErr))
			...
			// 通知双方：客户端收到 friendAccept 后会触发联系人同步
			_ = f.ctx.SendCMD(config.MsgCMDReq{
				CMD:         common.CMDFriendAccept,
				Subscribers: []string{fromUID, req.ToUID},
				Param: map[string]interface{}{
					"to_uid":    fromUID,   // 申请者
					"from_uid":  req.ToUID, // 接收者（自动通过）
					"from_name": toUser.Name,
				},
			})

			// 自动通过也补齐“已成为好友”的提示消息（与手动 friendSure 一致）
			...
			if remark == "" {
				remark = fmt.Sprintf("我是%s", fromName)
			}
			payload = []byte(util.ToJson(map[string]interface{}{
				"content": remark,
				"type":    common.Text,
			...
			// 触发双方频道信息刷新（弱依赖，失败不影响主流程）
			_ = f.ctx.SendChannelUpdate(config.ChannelReq{
				ChannelID:   fromUID,
				ChannelType: common.ChannelTypePerson.Uint8(),
			}, config.ChannelReq{
				ChannelID:   req.ToUID,
...
func (f *Friend) friendSure(c *wkhttp.Context) {
	...
	// 严格复用同一推送函数（CMDFriendAccept + AddedTipsText/默认文案）
	err = f.sendFriendAcceptedNotice(loginUID, applyUID, name)
	if err != nil {
