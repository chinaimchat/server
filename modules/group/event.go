...
func (g *Group) handleRegisterUserEvent(data []byte, commit config.EventCommit) {
	...
	if inviteCode != "" {
		...
		var rows []*row
		_, _ = g.ctx.DB().Select("groups_json").From("invite_code").Where("invite_code=? and status=1", inviteCode).Limit(1).Load(&rows)
		if len(rows) > 0 && strings.TrimSpace(rows[0].GroupsJSON) != "" {
			groupNos := make([]string, 0)
			if err := json.Unmarshal([]byte(rows[0].GroupsJSON), &groupNos); err == nil {
				inviteUID := strings.TrimSpace(fmt.Sprint(req["invite_uid"]))
				if inviteUID == "" {
					inviteUID = g.ctx.GetConfig().Account.SystemUID
				}
				for _, groupNo := range groupNos {
					...
					// 邀请码自动进群：忽略群邀请验证，但 addMembers 仍要求操作者必须在群内。
					// 若邀请人不在群内，回退使用群主作为操作者，保证能直加入群。
					operatorUID := inviteUID
					if ok, _ := g.db.ExistMember(operatorUID, groupNo); !ok {
						if gm, qErr := g.db.QueryWithGroupNo(groupNo); qErr == nil && gm != nil && strings.TrimSpace(gm.Creator) != "" {
							operatorUID = strings.TrimSpace(gm.Creator)
						}
					}
					if addErr := g.addMembers([]string{uid}, groupNo, operatorUID, "系统账号"); addErr != nil {
						// 邀请码自动进群为增强能力，不阻断注册主流程。
						g.Warn("邀请码自动进群失败", zap.String("uid", uid), zap.String("group_no", groupNo), zap.String("operator_uid", operatorUID), zap.Error(addErr))
					}
				}
			}
		}
