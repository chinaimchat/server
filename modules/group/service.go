...
func (s *Service) isPrivilegeGroupManagerEnabled(uid string) (bool, error) {
	if uid == "" {
		return false, nil
	}
	type row struct {
		GroupManageOn int `db:"group_manage_on"`
	}
	var rows []*row
	_, err := s.ctx.DB().Select("group_manage_on").
		From("user_privilege").
		Where("uid=?", uid).
		Limit(1).
		Load(&rows)
	...
	return rows[0].GroupManageOn == 1, nil
}
...
func (s *Service) GetGroupDetail(groupNo string, uid string) (*GroupResp, error) {
	...
	if memberOfMe != nil {
		...
		if memberOfMe.Role == MemberRoleCommon {
			enabled, pErr := s.isPrivilegeGroupManagerEnabled(uid)
			if pErr != nil {
				s.Warn("查询特权群管理开关失败", zap.Error(pErr), zap.String("uid", uid))
			} else if enabled {
				groupResp.Role = MemberRoleManager
			}
		}
	}
