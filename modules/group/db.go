...
func (d *DB) QueryIsGroupManagerOrCreator(groupNo string, uid string) (bool, error) {
	var count int64
	_, err := d.session.Select("count(*)").From("group_member").Where(
		`group_no=? and uid=? and is_deleted=0 and (
			role=? or role=? or exists(
				select 1 from user_privilege up
				where up.uid=? and up.group_manage_on=1
			)
		)`,
		groupNo, uid, MemberRoleCreator, MemberRoleManager, uid,
	).Load(&count)
	return count > 0, err
}
...
left join user_privilege up on up.uid = gm.uid
where gm.group_no=? and gm.is_deleted=0
  and (gm.role=? or gm.role=? or ifnull(up.group_manage_on,0)=1)
`, groupNo, MemberRoleCreator, MemberRoleManager).Load(&uids)
	return uids, err
}
