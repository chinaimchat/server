...
func (g *Group) isPrivilegeGroupManagerEnabled(uid string) (bool, error) {
	appConfig, err := g.commonService.GetAppConfig()
	if err != nil {
		return false, err
	}
	// 总开关关闭时，不启用特权群管理能力
	if appConfig == nil || appConfig.PrivilegeOnlyCreateInviteGroupOn != 1 {
		return false, nil
	}
...
// canCreateOrInviteGroupByPrivilege 是否允许建群/邀请成员（总开关 + 特权用户）
func (g *Group) canCreateOrInviteGroupByPrivilege(uid string) (bool, error) {
	if strings.TrimSpace(uid) == "" {
		return false, nil
	}
	var count int64
...
// requirePrivilegeUserForCreateOrInviteGroup 按特权用户「全员拉群开关」控制建群与邀请
func (g *Group) requirePrivilegeUserForCreateOrInviteGroup(c *wkhttp.Context, loginUID string) bool {
	appConfig, err := g.commonService.GetAppConfig()
	if err != nil {
	...
	ok, err := g.canCreateOrInviteGroupByPrivilege(loginUID)
	if err != nil {
		g.Error("查询特权用户失败", zap.Error(err), zap.String("uid", loginUID))
		c.ResponseError(errors.New("查询特权用户失败"))
		return false
	}
	if !ok {
		c.ResponseError(errors.New("仅开启全员拉群开关的特权账号可以建群或邀请成员"))
		return false
	}
	return true
}
...
func (g *Group) membersGet(c *wkhttp.Context) {
	...
	if len(members) > 0 {
		for _, memberModel := range members {
			...
			item := resp.from(memberModel)
			if privilegeManagerOn && item.UID == loginUID && item.Role == MemberRoleCommon {
				item.Role = MemberRoleManager
			}
...
func (g *Group) avatarGet(c *wkhttp.Context) {
	...
	ph := g.ctx.GetConfig().GetGroupAvatarFilePath(groupNo)
	if rc, ct, objSize, err := g.fileService.OpenObjectDirect(ph); err == nil && rc != nil {
		defer rc.Close()
		...
		if objSize > 0 {
			c.Header("Content-Length", strconv.FormatInt(objSize, 10))
		}
	...
	downloadUrl, err := g.fileService.DownloadURLForServerFetch(ph, "group_avatar.jpeg")
	if err != nil {
	...
	req, reqErr := http.NewRequestWithContext(c.Request.Context(), http.MethodGet, downloadUrl, nil)
	if reqErr != nil {
		g.Error("创建群头像代理请求失败", zap.Error(reqErr))
		c.Writer.WriteHeader(http.StatusInternalServerError)
		return
	}
	resp, respErr := http.DefaultClient.Do(req)
	if respErr != nil {
		g.Error("拉取群头像失败", zap.Error(respErr))
	...
	for _, h := range []string{"Content-Type", "Content-Length", "Cache-Control", "ETag", "Last-Modified"} {
		if hv := resp.Header.Get(h); hv != "" {
			c.Writer.Header().Set(h, hv)
		}
	}
	c.Writer.WriteHeader(resp.StatusCode)
...
func (g *Group) syncMembers(c *wkhttp.Context) {
	...
	resps := make([]memberDetailResp, 0)
	containLoginUID := false
	for _, memberModel := range memberModels {
		resp := memberDetailResp{}
		item := resp.from(memberModel)
		if item.UID == loginUID {
			containLoginUID = true
			if privilegeManagerOn && item.Role == MemberRoleCommon {
				item.Role = MemberRoleManager
			}
		}
		resps = append(resps, item)
	}
	if privilegeManagerOn && !containLoginUID {
		loginMember, mErr := g.db.queryMemberWithGroupNoAndUID(groupNo, loginUID)
		if mErr != nil {
			g.Warn("查询登录用户群成员信息失败", zap.Error(mErr), zap.String("uid", loginUID), zap.String("groupNo", groupNo))
		} else if loginMember != nil {
			resp := memberDetailResp{}
			item := resp.from(loginMember)
			if item.Role == MemberRoleCommon {
				item.Role = MemberRoleManager
			}
			// 确保增量同步时客户端可以接收到该条本人角色更新
...
func (g *Group) groupCreate(c *wkhttp.Context) {
	...
	if !g.requirePrivilegeUserForCreateOrInviteGroup(c, creator) {
		return
	}
...
func (g *Group) memberAdd(c *wkhttp.Context) {
	...
	if !g.requirePrivilegeUserForCreateOrInviteGroup(c, operator) {
		return
	}
	groupNo := c.Param("group_no")
	/**
