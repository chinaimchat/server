...
// Twemoji 72x72 文件名（与前端 Unicode codepoint 连字符规则一致）
var twemoji72PNGNameRe = regexp.MustCompile(`^[0-9a-fA-F-]+\.png$`)
...
func (cn *Common) Route(r *wkhttp.WKHttp) {
	...
	commonNoAuth := r.Group("/v1/common")
	{
		...
		// Twemoji 表情图：客户端只请求本 API；可选 TWEMOJI_72_DIR 本地目录，否则服务端从上游拉取
		commonNoAuth.GET("/twemoji72/:name", cn.twemoji72PNG)
	}
...
// twemoji72PNG 提供 Twemoji 72×72 PNG，供 Web/PC 表情列表与消息内嵌表情统一走 API 域名。
// 优先级：
//  1. 环境变量 TWEMOJI_72_DIR 下同名文件（运维显式指定路径）；
//  2. 进程工作目录下 ./assets/twemoji72/（镜像内置目录，离线部署即用）；
//  3. GET TWEMOJI_72_UPSTREAM（默认 jsDelivr）并回传（需要出网）。
func (cn *Common) twemoji72PNG(c *wkhttp.Context) {
	name := filepath.Base(c.Param("name"))
	if name == "." || name == "/" || !twemoji72PNGNameRe.MatchString(name) {
		c.Writer.WriteHeader(http.StatusBadRequest)
		return
	}
	candidates := make([]string, 0, 2)
	if dir := strings.TrimSpace(os.Getenv("TWEMOJI_72_DIR")); dir != "" {
		candidates = append(candidates, filepath.Join(dir, name))
	}
	// 约定的镜像内置目录：Dockerfile 在构建时 COPY 进 /home/assets/twemoji72/，工作目录即 /home。
	candidates = append(candidates, filepath.Join("assets", "twemoji72", name))
	...
	up := strings.TrimSpace(os.Getenv("TWEMOJI_72_UPSTREAM"))
	if up == "" {
		up = "https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72"
	}
	up = strings.TrimRight(up, "/") + "/"
	client := &http.Client{Timeout: 12 * time.Second}
	resp, err := client.Get(up + name)
	if err != nil {
		cn.Warn("twemoji upstream get failed", zap.String("name", name), zap.Error(err))
	...
	if err != nil {
		cn.Warn("twemoji upstream read failed", zap.String("name", name), zap.Error(err))
		c.Writer.WriteHeader(http.StatusBadGateway)
		return
	}
