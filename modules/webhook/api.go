...
func (w *Webhook) handleMsgOffline(data []byte) error {
	...
	if msgResp.Compress == "gzip" {
		if len(msgResp.CompresssToUIDs) > 0 {
			...
			defer gReader.Close()
			compressToUIDBytes, err := io.ReadAll(gReader)
			if err != nil {
				w.Error("读取gzip压缩数据失败！", zap.Error(err))
				return err
			}
			err = util.ReadJsonByByte(compressToUIDBytes, &toUids)
			if err != nil {
				w.Error("")
				return err
			}
...
func (w *Webhook) push(toUser *user.Resp, msgResp msgOfflineNotify) (pushResp, error) {
	...
	if len(deviceMap) <= 0 {
		w.Debug("跳过离线推送：无已注册推送设备", zap.String("uid", toUID))
		return pushResp{}, nil
	}
	...
	if w.pushMap[common.DeviceType(deviceType)] == nil {
		w.Debug("跳过离线推送：服务端未配置该设备类型的推送通道", zap.String("uid", toUID), zap.String("deviceType", deviceType))
		return pushResp{
	...
	if pusher == nil {
		w.Debug("跳过离线推送：bundle 与推送配置不匹配", zap.String("uid", toUID), zap.String("deviceType", deviceType), zap.String("bundleID", bundleID))
		return pushResp{
			deviceType:  deviceType,
			deviceToken: deviceToken,
		}, nil
