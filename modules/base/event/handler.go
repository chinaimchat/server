...
func (e *Event) handleGroupAvatarUpdateEvent(model *Model) {
	...
	e.ctx.EventPool.Work <- &pool.Job{
		...
		JobFunc: func(id int64, data interface{}) {
			...
			// 必须用本机回环，勿用 External 公网地址；容器内访问 192.x.x.x:8090 常超时，导致拼图失败、群相关图片异常。
			apiBaseURL := wkutil.LoopbackAPIBaseURL(e.ctx.GetConfig().Addr)
			downloadURLs := make([]string, 0)
			for _, member := range req.Members {
				if len(downloadURLs) >= 9 {
					break
				}
				downloadURLs = append(downloadURLs, wkutil.FullAPIURL(apiBaseURL, wkutil.AvatarAPIRelativePath(member)))
			}
