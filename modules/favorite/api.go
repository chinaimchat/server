...
func (f *Favorite) Route(r *wkhttp.WKHttp) {
	favorites := r.Group("/v1/favorites", f.ctx.AuthMiddleware(r))
	{
		favorites.POST("", f.add)               // 添加收藏
		favorites.POST("/update/:id", f.update) // 修改收藏
		favorites.DELETE("/:id", f.delete)      //删除收藏
	}
	favorite := r.Group("/v1/favorite", f.ctx.AuthMiddleware(r))
	{
...
// 更新收藏
func (f *Favorite) update(c *wkhttp.Context) {
	loginUID := c.MustGet("uid").(string)
	id := c.Param("id")
	...
	record, err := f.db.QueryCollectByUniqueKey(id, loginUID)
	if err != nil || record == nil {
		c.ResponseError(errors.New("记录不存在！"))
		return
	}
	for k, v := range reqMap {
		if k != "unique_key" {
			if k == "payload" {
				...
				data := string(b[:])
				f.db.UpdateFavoriteWithField(k, data, id, loginUID)
			} else {
				//f.db.UpdateFavoriteWithField(k, string, id, loginUID)
			}
		}
	}
	c.ResponseOK()
