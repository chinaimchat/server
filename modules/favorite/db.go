...
// UpdateFavoriteWithField 修改收藏
func (d *DB) UpdateFavoriteWithField(field string, value string, uniqueKey string, uid string) error {
	_, err := d.session.Update("favorite").Set(field, value).Where("unique_key=? and uid=?", uniqueKey, uid).Exec()
	return err
}
