...
// 添加表情
func (d *DB) insertStickerTx(m *model, tx *dbr.Tx) error {
	_, err := tx.InsertInto("sticker").Columns(util.AttrToUnderscore(m)...).Record(m).Exec()
	return err
}

// 添加表情
func (d *DB) insertSticker(m *model) error {
...
// 添加用户表情
func (d *DB) insertUserStickerTx(m *customModel, tx *dbr.Tx) error {
	_, err := tx.InsertInto("sticker_custom").Columns(util.AttrToUnderscore(m)...).Record(m).Exec()
	return err
}

// 添加用户表情
func (d *DB) insertUserSticker(m *customModel) error {
	_, err := d.session.InsertInto("sticker_custom").Columns(util.AttrToUnderscore(m)...).Record(m).Exec()
	return err
}
...
// 添加用户表情分类
func (d *DB) insertUserCategory(m *categoryModel) error {
	_, err := d.session.InsertInto("sticker_user_category").Columns(util.AttrToUnderscore(m)...).Record(m).Exec()
	return err
}
...
func (d *DB) queryStickerStroeWithPage(pageIndex, pageSize uint64, uid string) ([]*stickerStoreModel, error) {
	...
	// 显式选择字段并做 alias，避免 gocraft/dbr 在遇到 `desc`/`cover_lim` 等字段映射时出现不一致。
	// 安卓端会直接使用 title/desc，因此这里必须保证字段落到正确的结构字段上。
	_, err := d.session.
		Select(
			"sticker_store.id",
			"sticker_store.category",
			"sticker_store.title",
			"sticker_store.`desc` `desc`",
			"sticker_store.cover",
			"sticker_store.cover_lim cover_lim",
			"sticker_store.created_at",
			"sticker_store.updated_at",
			"IF(sticker_user_category.id>0,1,0) status",
		).
		From("sticker_store").
		Where("sticker_store.is_gone=0").
		LeftJoin("sticker_user_category", str).
		OrderDir("sticker_store.created_at", false).
...
// 添加表情商店
func (d *DB) insertStore(m *storeModel) error {
	_, err := d.session.InsertInto("sticker_store").Columns(util.AttrToUnderscore(m)...).Record(m).Exec()
	return err
}
