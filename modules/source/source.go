...
func GetSoruce(code string) string {
	...
	case common.QRCode:
		return "通过扫一扫添加"
	case common.InvitationCode:
		return "通过邀请码注册添加"
	case common.GroupMember:
...
func CheckSource(code string) error {
	...
	if code == "" || strings.EqualFold(code, "<nil>") || strings.EqualFold(code, "nil") {
	...
	codeTypeStr := strings.TrimSpace(strs[len(strs)-1])
	if codeTypeStr == "" {
		return errors.New("来源错误")
	}
	codeTypeInt, err := strconv.Atoi(codeTypeStr)
	if err != nil {
		return errors.New("来源错误")
	}
	codeType := common.VercodeType(codeTypeInt)
	if codeType != common.Friend && codeType != common.QRCode && codeType != common.User && codeType != common.GroupMember && codeType != common.MailList && codeType != common.InvitationCode {
		return errors.New("来源错误")
	}
	switch codeType {
